...
export function JobActions({
  ...
  const canResume =
    job.is_paused && !["completed", "cancelled"].includes(job.status);
  const canCancel = !["completed", "cancelled"].includes(job.status);
  const canRetry = ["failed", "cancelled"].includes(job.status);
  const hasNormalizedTranscript = job.artifacts.some(
  ...
  const canReclean = hasNormalizedTranscript;
      ...
      <div className="flex flex-wrap items-center gap-2">
        {textArtifact ? (
          <Button
            ...
            variant={finalArtifact ? "default" : "secondary"}
          >
            <Link to={`/text/${job.id}`}>
              <TextIcon />
              View text
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {job.source_available || textArtifact ? (
              <DropdownMenuSeparator />
            ) : null}
            {canPause ? (
              <DropdownMenuItem
                onSelect={() => void run(() => pauseJob(job.id))}
              >
                <PauseIcon />
                Pause
              </DropdownMenuItem>
            ...
            {canResume ? (
              <DropdownMenuItem
                onSelect={() => void run(() => resumeJob(job.id))}
              >
                <RotateCcwIcon />
                Resume
              </DropdownMenuItem>
            ...
            {canRetry ? (
              <DropdownMenuItem
                onSelect={() =>
                  void run(() => retryJob(job.id), "Job queued again.")
                }
              >
                <RefreshCcwIcon />
                Retry
            ...
            {canReclean ? (
              <DropdownMenuItem
                ...
                onSelect={() => setConfirmReclean(true)}
              >
                <RefreshCcwIcon />
                {cleaningBusy
