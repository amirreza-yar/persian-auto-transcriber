...
export function FileItem({
  ...
  const toggleOffline = async () => {
    ...
    try {
      if (offlineSaved) {
        ...
        toast.success(
          "Audio, cleaned text and edited text saved for offline use.",
        );
      }
      onOfflineChanged?.();
    } catch (error) {
  ...
  const hasNormalizedTranscript =
    job?.artifacts.some(
      (artifact) => artifact.kind === "subtitle_normalized_json",
    ) ?? false;
  const cleaningBusy =
    job?.tasks.some(
      (task) =>
        task.kind === "clean_text" &&
        ["queued", "retry_wait", "claimed", "running"].includes(task.status),
    ) ?? false;
  const canReclean = Boolean(job && hasNormalizedTranscript);
  ...
  return (
    <>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-3">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  {canReclean ? (
                    <DropdownMenuItem
                      disabled={busy || cleaningBusy}
                      onSelect={() => setRecleanOpen(true)}
                    >
                      <RefreshCcwIcon />
                      {cleaningBusy
                        ? "Cleanup already queued/running"
                        : "Re-clean transcript"}
                    </DropdownMenuItem>
                  ) : null}
                  {file.source_available ? (
                    <DropdownMenuItem asChild>
                      <a href={audioDownloadUrl(file.id)}>
                        <DownloadIcon />
                        Download audio
                      </a>
                    </DropdownMenuItem>
                  ...
                  {file.source_available &&
                  ["completed", "cancelled"].includes(file.job_status) ? (
                    <>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        variant="destructive"
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              {job ? (
                ...
                <Badge variant="outline">{file.job_status}</Badge>
              )}
              {offlineSaved ? (
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {file.source_available || offlineSaved ? (
                <Button asChild size="sm" variant="outline">
                  <Link to={`/player/${file.id}`}>
                    <PlayIcon />
                    Play
                  </Link>
                </Button>
              ) : null}
              {textArtifact || offlineSaved ? (
                <Button asChild size="sm">
                  <Link to={`/text/${file.job_id}`}>
                    <TextIcon />
                    View text
                  </Link>
                </Button>
              ...
              {textArtifact ? (
                <Button asChild size="sm" variant="outline">
                  <a href={artifactDownloadUrl(textArtifact.id)}>
                    <DownloadIcon />
                    Download text
                  </a>
                </Button>
      ...
      <ConfirmDialog
        ...
        title="Delete the source audio?"
        description="The original recording will be removed from the server. Existing transcript and subtitle files will stay available."
